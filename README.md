...
### 8.5 内网穿透（公网入口 → 内网节点）不通

> 内网穿透的“最常见失败点”通常不是规则本身，而是**端口未放行 / TLS 明文不匹配 / 内网节点未拿到配置**。

1）在 **公网入口节点(A)** 检查：
- **入口监听端口**（你填的 listen，比如 `0.0.0.0:2222`）是否已放行防火墙/安全组
- **隧道端口**（默认 `18443`，可在规则里改）是否已放行（只需内网节点(B)能访问）
...
3）查看两端“诊断状态”（新版会给出 warnings）：
```bash
curl -s -H "X-API-Key: $(cat /etc/realm-agent/api.key)" http://127.0.0.1:18700/api/v1/intranet/status | jq
```
- A 侧：如果 `warnings` 提示“未检测到内网客户端连接”，说明 B 还没连上（网络/端口/配置下发问题）
- B 侧：如果提示“客户端未连接到 ...（dial_tls_failed / dial_failed）”，多半是端口不可达或 TLS/明文不匹配

4）看内网穿透专用日志（两端都有）：
```bash
tail -n 200 /etc/realm-agent/intranet/intranet.log
```

5）若面板提示“直连下发失败”：
- 这通常表示节点在内网，面板无法直接访问它的 Agent 端口。
- 只要节点能访问面板并持续上报（push-report），规则会在**下一次上报**自动同步生效。
