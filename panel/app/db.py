...
-- Traffic history for rules (stored per-rule, per-node)
...
CREATE INDEX IF NOT EXISTS idx_traffic_history_node_rule_ts ON traffic_history(node_id, rule_index, ts_ms);
"""
...
def insert_traffic_history(
    ...
    """Insert a traffic history record."""
    with connect(db_path) as conn:
        cur = conn.execute(
            """
            INSERT INTO traffic_history 
            (node_id, rule_index, ts_ms, connections, connections_active, connections_total, rx_bytes, tx_bytes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (node_id, rule_index, ts_ms, connections, connections_active, connections_total, rx_bytes, tx_bytes),
        )
...
def list_traffic_history(
    ...
    """List traffic history records."""
    with connect(db_path) as conn:
        if rule_index is not None:
            rows = conn.execute(
                """
                SELECT * FROM traffic_history
                WHERE node_id = ? AND rule_index = ? AND ts_ms >= ?
                ORDER BY ts_ms DESC
                LIMIT ?
                """,
                (node_id, rule_index, since_ts_ms, limit),
            ).fetchall()
        else:
            rows = conn.execute(
                """
                ...
                LIMIT ?
                """,
                (node_id, since_ts_ms, limit),
            ).fetchall()
    return [dict(r) for r in rows]


def list_traffic_history_rollup(
    node_id: int,
    since_ts_ms: int = 0,
    bucket_ms: int = 60000,
    rule_index: Optional[int] = None,
        ...
        if rule_index is not None:
            ...
            sql = """
                SELECT
                    ...
                    SUM(connections_total) AS connections_total,
...
def prune_traffic_history(before_ts_ms: int, db_path: str = DEFAULT_DB_PATH) -> int:
    """Prune old traffic history records."""
    try:
        cutoff = int(before_ts_ms)
    except Exception:
        cutoff = 0
    if cutoff <= 0:
        return 0
    with connect(db_path) as conn:
        cur = conn.execute("DELETE FROM traffic_history WHERE ts_ms < ?", (cutoff,))
        conn.commit()
        return int(cur.rowcount or 0)
