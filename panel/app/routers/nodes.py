...
async def node_detail_page(request: Request, node_id: int, user: str = Depends(require_login_page)):
    """节点详情页面"""
    from ..main import templates
    from ..db import get_group_orders, list_nodes
    ...
    # Build node groups for sidebar
    all_nodes = list_nodes()
    group_orders = get_group_orders()
    groups_dict = {}
    
    for n in all_nodes:
        gn = n.get("group_name", "默认分组") or "默认分组"
        if gn not in groups_dict:
            groups_dict[gn] = {"name": gn, "nodes": [], "online": 0, "total": 0}
        groups_dict[gn]["nodes"].append(n)
        groups_dict[gn]["total"] += 1
        if _is_report_fresh(n):
            groups_dict[gn]["online"] += 1
    
    # Apply sort order
    for gn, order in group_orders.items():
        if gn in groups_dict:
            groups_dict[gn]["sort_order"] = order
